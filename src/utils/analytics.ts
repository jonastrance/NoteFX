...
import { FocusSession, Goal, Note, Task } from '../types/analytics';
...
export interface TagUsagePoint {
  tag: string;
  count: number;
}
...
export interface NoteLengthStats {
...
export const calculateNoteFrequency = (notes: Note[], start: Date, end: Date): FrequencyPoint[] => {
...
export const calculateTaskCompletionMetrics = (tasks: Task[]): TaskCompletionMetrics => {
...
export const calculateFocusMetrics = (sessions: FocusSession[]): FocusMetrics => {
...
export const calculateTagUsage = (notes: Note[]): TagUsagePoint[] => {
...
const getPeriodKey = (date: string, view: TrendView): string => {
  const d = dayjs(date);
  switch (view) {
    case 'weekly':
...
export const calculateProductivityTrends = (
  ...
  const map = new Map<string, ProductivityTrendPoint>();

  const ensureEntry = (period: string) => {
    if (!map.has(period)) {
...
export const calculateNoteLengthStats = (notes: Note[]): NoteLengthStats => {
  if (notes.length === 0) {
    return {
      averageWordCount: 0,
      medianWordCount: 0,
  ...
  const medianWordCount =
    wordCounts.length % 2 === 0 ? (wordCounts[middle - 1] + wordCounts[middle]) / 2 : wordCounts[middle];
  const minWordCount = wordCounts[0];
  const maxWordCount = wordCounts[wordCounts.length - 1];
  const averageReadingTime = Number(
    (
      notes.reduce((acc, note) => acc + estimateReadingTime(note.wordCount), 0) /
      notes.length
    ).toFixed(1)
  );
...
export const calculateGoalProgress = (goals: Goal[]): GoalProgress[] =>
  goals.map((goal) => {
...
export const buildComparisonMetrics = (
  ...
  const build = (metric: string, cur: number, prev: number) => {
...
export const prepareCsvData = (notes: Note[], tasks: Task[], sessions: FocusSession[]): string => {
...
export const prepareJsonData = (notes: Note[], tasks: Task[], sessions: FocusSession[], goals: Goal[]) =>
  JSON.stringify(
    {
