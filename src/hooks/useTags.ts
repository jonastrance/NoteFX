...
const getHierarchyDepth = (tags: Tag[]) => {
  const map = new Map<string, Tag>();
  tags.forEach((tag) => map.set(tag.id, tag));

  const depthMemo = new Map<string, number>();
  const computeDepth = (tag: Tag): number => {
    if (!tag.parentId) return 1;
    if (depthMemo.has(tag.id)) return depthMemo.get(tag.id)!;
...
export const useTags = () => {
  ...
  const search = (query: string) => {
    ...
    return state.tags.filter((tag) =>
      `${tag.name} ${tag.description ?? ''}`.toLowerCase().includes(lower)
    );
  };

  const getChildren = (parentId: string) => state.tags.filter((tag) => tag.parentId === parentId);
