...
export const useProductivityMetrics = ({ notes, tasks, focusSessions, focusMetrics, taskMetrics, dateRange }) => {
    ...
    const averageDailyFocusMinutes = useMemo(() => Number((focusMetrics.totalMinutes / totalDays).toFixed(1)), [focusMetrics.totalMinutes, totalDays]);
    const focusEfficiency = useMemo(() => {
        if (tasks.length === 0 || focusMetrics.totalMinutes === 0)
            return 0;
    ...
    const streak = useMemo(() => calculateStreak(notes.map((note) => new Date(note.createdAt))), [notes]);
